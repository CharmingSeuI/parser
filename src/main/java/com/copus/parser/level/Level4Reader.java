...
public class Level4Reader {
    ...
    public void read(Document doc) {
        ...
        for (int level4Index = 0; level4Index < level4s.getLength(); level4Index++) {
            ...
            String DCI = Optional.ofNullable(level4Attributes.getNamedItem("DCI")).map(n -> n.getNodeValue()).orElse("null");
            ...
            AnnotationInfo annotationInfo = Optional.ofNullable(InfoIdRepository.annotationInfoId.get(level_4_id))
                    .map(n -> em.find(AnnotationInfo.class, n)).orElse(null);
            BodyInfo bodyInfo = Optional.ofNullable(InfoIdRepository.bodyInfoId.get(level_4_id))
                    .map(n -> em.find(BodyInfo.class, n)).orElse(null);
            ConnectionInfo connectionInfo = Optional.ofNullable(InfoIdRepository.connectionInfoId.get(level_4_id))
                    .map(n -> em.find(ConnectionInfo.class, n)).orElse(null);
            MetaInfo metaInfo = Optional.ofNullable(InfoIdRepository.metaInfoId.get(level_4_id))
